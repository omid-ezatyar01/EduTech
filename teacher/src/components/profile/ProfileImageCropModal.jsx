...
export default function ProfileImageCropModal({
  ...
  const t = {
    ...
    subtitle: isFa
      ? "عکس را جابه‌جا و بزرگ‌نمایی کنید تا دقیقاً همان بخشی که می‌خواهید در پروفایل دیده شود انتخاب گردد."
      : "Move and zoom the image to choose the exact area that should appear in the profile.",
    move: isFa ? "جابه‌جایی" : "Move",
    zoom: isFa ? "بزرگ‌نمایی" : "Zoom",
    cancel: isFa ? "انصراف" : "Cancel",
    apply: isFa ? "ذخیره این برش" : "Use this crop",
    loading: isFa ? "در حال آماده‌سازی تصویر..." : "Preparing image",
    failed: isFa ? "آماده‌سازی تصویر انجام نشد." : "Unable to prepare the image.",
    helper: isFa
      ? "خروجی نهایی به شکل عکس پروفایل شما ذخیره می‌شود."
      : "The final output will be saved as your profile image.",
  };
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      const size = Math.min(element.clientWidth || 0, 360);
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-end justify-center bg-[#0F172A]/75 p-0 sm:items-center sm:p-4">
      <div
        className="w-full max-w-3xl rounded-t-[28px] bg-white shadow-2xl sm:rounded-[30px]"
        dir={isFa ? "rtl" : "ltr"}
      >
        ...
        <div className="space-y-5 p-5 sm:p-6">
          <div className="rounded-[28px] border border-slate-200 bg-[radial-gradient(circle_at_top,#f8fafc,#eef2ff)] p-4 sm:p-6">
            <div ref={frameRef} className="mx-auto w-full max-w-[360px]">
              <div className="relative aspect-square overflow-hidden rounded-[32px] bg-slate-900 shadow-[0_24px_55px_rgba(15,23,42,0.18)]">
                {previewUrl ? (
                  <div
                ...
                <div className="pointer-events-none absolute inset-0 rounded-[32px] border-[14px] border-white/55" />
          ...
          <div className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-[auto_1fr] sm:items-center">
            <div className="inline-flex items-center gap-2 text-sm font-black text-slate-700">
              <Move size={16} className="text-[#0B4FD8]" />
              {t.move}
            </div>
            <p className="text-xs font-semibold text-slate-500">{t.helper}</p>

            <div className="inline-flex items-center gap-2 text-sm font-black text-slate-700">
              <Plus size={16} className="text-[#0B4FD8]" />
              {t.zoom}
            </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            <button
              ...
              className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#0B4FD8] px-5 text-sm font-black text-white shadow-lg shadow-blue-200 transition hover:bg-[#0A45BE] disabled:cursor-not-allowed disabled:opacity-60"
            >
