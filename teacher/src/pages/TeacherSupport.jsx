...
const copy = {
  en: {
    ...
    subject: "Subject", category: "Category", help: "Describe your question", create: "Start conversation",
    close: "Close ticket", reopen: "Reopen", live: "Live", reconnecting: "Reconnecting",
  ...
  fa: {
    ...
    subject: "موضوع", category: "دسته‌بندی", help: "پرسش خود را توضیح دهید", create: "شروع گفتگو",
    close: "بستن تکت", reopen: "بازکردن دوباره", live: "زنده", reconnecting: "در حال اتصال",
    statuses: { open: "باز", in_progress: "در حال بررسی", waiting_for_user: "منتظر پاسخ شما", resolved: "حل‌شده", closed: "بسته" },
    categories: { account: "حساب", course: "کورس", payment: "پرداخت", technical: "مشکل فنی", teaching: "تدریس", certificate: "سرتیفیکیت", other: "سایر" },
  },
};
...
export default function TeacherSupport() {
  ...
  const [form, setForm] = useState({ subject: "", category: "teaching", message: "" });
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => { loadList().catch(() => {}); if (selectedId) loadChat(selectedId).catch(() => {}); }, 30_000);
  ...
  const submitTicket = async (event) => {
    ...
    try {
      ...
      setCreating(false); setForm({ subject: "", category: "teaching", message: "" });
  ...
  const toggle = async () => {
    await setSupportTicketOpenState(selectedId, chat.ticket.status === "closed" ? "open" : "closed");
