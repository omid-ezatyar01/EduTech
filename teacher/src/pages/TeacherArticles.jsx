...
export default function TeacherArticles() {
  ...
  const edit = (item) => { const preferredLanguage = language === "fa" ? "fa" : "en"; setEditingId(item._id); setEditorLanguage(item.title?.[preferredLanguage] ? preferredLanguage : preferredLanguage === "fa" ? "en" : "fa"); setForm(toForm(item)); setOpen(true); };
  ...
  const selectCover = async (event) => { const file = event.target.files?.[0]; event.target.value = ""; if (!file) return; setUploadingCover(true); setError(""); try { updateField("coverImage", await uploadTeacherArticleCover(file)); } catch (err) { setError(err.message || text.saveError); } finally { setUploadingCover(false); } };

  return <TeacherLayout teacher={teacher} language={language} onLanguageChange={setLanguage}><div className="mx-auto max-w-7xl">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-3xl font-black text-slate-950">{text.title}</h1><p className="mt-2 max-w-3xl text-sm font-semibold leading-7 text-slate-500">{text.subtitle}</p></div><button type="button" onClick={() => { setEditingId(""); setEditorLanguage(language === "fa" ? "fa" : "en"); setForm(EMPTY); setOpen(true); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0B4FD8] px-5 py-3 text-sm font-black text-white"><Plus size={18}/>{text.add}</button></div>
