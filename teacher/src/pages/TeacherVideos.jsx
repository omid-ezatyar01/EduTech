...
export default function TeacherVideos() {
  ...
  return <TeacherLayout teacher={teacher} language={language} onLanguageChange={setLanguage}>
    ...
    {loading ? <TeacherPageLoader label={text.load}/> : videos.length === 0 ? <div className="mt-7 rounded-3xl border border-dashed border-slate-300 bg-white py-20 text-center"><Video className="mx-auto text-slate-300" size={48}/><p className="mt-3 font-bold text-slate-500">{text.empty}</p></div> : <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{videos.map((item) => <article key={item._id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="aspect-video bg-white bg-center bg-no-repeat" style={{ backgroundImage: 'url("/logo.png")', backgroundSize: "72% auto" }}>{item.thumbnailUrl ? <img src={item.thumbnailUrl} alt="" loading="lazy" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.hidden = true; }}/> : null}</div><div className="p-5"><div className="flex items-start justify-between gap-3"><h2 className="font-black text-slate-950">{item.title}</h2><span className={`rounded-full px-2 py-1 text-[11px] font-black ${item.isPublished ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{item.isPublished ? text.published : text.hidden}</span></div><div className="mt-4 flex gap-2"><button type="button" onClick={() => edit(item)} className="inline-flex items-center gap-1 rounded-lg bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700"><Pencil size={14}/>{text.edit}</button><button type="button" onClick={() => toggle(item)} className="rounded-lg bg-slate-100 p-2 text-slate-600">{item.isPublished ? <EyeOff size={15}/> : <Eye size={15}/>}</button><button type="button" onClick={() => remove(item)} className="rounded-lg bg-red-50 p-2 text-red-600"><Trash2 size={15}/></button></div></div></article>)}</div>}
