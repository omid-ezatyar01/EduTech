...
export const getCategoryParentOptions = (rows = [], editingId = "") => {
  const blockedIds = new Set([String(editingId || "").trim()]);
  getSubcategoriesForParent(rows, editingId).forEach((item) => {
    blockedIds.add(String(item?._id || ""));
  });

  return (Array.isArray(rows) ? rows : [])
    .filter((item) => !blockedIds.has(String(item?._id || "")))
    .sort((left, right) =>
      formatCategoryPathLabel(left).localeCompare(formatCategoryPathLabel(right)),
    );
};
