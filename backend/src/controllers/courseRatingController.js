...
export const submitStudentCourseRating = asyncHandler(async (req, res) => {
  ...
  const previousTeacherRating = await CourseRating.findOne({
    studentId: req.user._id,
    teacherId: prompt.teacherId,
  }).select("teacherRating").lean();
  const teacherRating = previousTeacherRating
    ? Number(previousTeacherRating.teacherRating)
    : normalizeRating(req.body?.teacherRating, "teacherRating");
...
export const updateStudentRating = asyncHandler(async (req, res) => {
  ...
  await row.save();
  await CourseRating.updateMany(
    { studentId: req.user._id, teacherId: row.teacherId, _id: { $ne: row._id } },
    { $set: { teacherRating: row.teacherRating } },
  );
...
export const getTeacherRatingInsights = asyncHandler(async (req, res) => {
  const rows = await CourseRating.find({ teacherId: req.user._id, removedByTeacherAt: null })
  ...
  rows.forEach((row) => {
    ...
    if (studentId && !uniqueByStudent.has(studentId)) uniqueByStudent.set(studentId, row);
  });
  const uniqueRatings = [...uniqueByStudent.values()];
  const distribution = [1, 2, 3, 4, 5].reduce((acc, value) => ({ ...acc, [value]: uniqueRatings.filter((row) => row.teacherRating === value).length }), {});
  const average = uniqueRatings.length ? uniqueRatings.reduce((sum, row) => sum + Number(row.teacherRating || 0), 0) / uniqueRatings.length : 0;
  return res.json(new ApiResponse({ message: "Teacher feedback fetched successfully", data: { average: Math.round(average * 10) / 10, total: uniqueRatings.length, distribution, reviews: rows.map(ratingPayload) } }));
});

export const replyToTeacherRating = asyncHandler(async (req, res) => {
  const row = await CourseRating.findOne({ _id: req.params.id, teacherId: req.user._id, removedByTeacherAt: null });
  if (!row) throw new ApiError(404, "Rating not found");
...
export const setTeacherRatingVisibility = asyncHandler(async (req, res) => {
  ...
  return res.json(new ApiResponse({ message: hidden ? "Review hidden successfully" : "Review sent for moderation", data: ratingPayload(row) }));
});
