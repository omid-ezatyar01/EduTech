...
export default function CertificateFilterBar({
  ...
  const searchPlaceholder = isFa
    ? "جستجو در سرتیفیکیت‌ها ..."
    : "Search certificates...";
  const normalizedOptions = [courses, statuses, sorts].map((options) =>
  ...
  return (
    <div className="mb-6 grid gap-4 rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_auto_auto_auto]">
      ...
      {[
        { value: courseFilter, setter: setCourseFilter, options: normalizedOptions[0] },
        { value: statusFilter, setter: setStatusFilter, options: normalizedOptions[1] },
