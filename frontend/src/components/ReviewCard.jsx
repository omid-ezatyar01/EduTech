...
export default function ReviewCard({ review }) {
  ...
  const isUnpublishedOwnReview = Boolean(
    review?.isOwnReview && review?.moderationStatus !== "published",
  );
  const reviewStatusLabel = review?.moderationStatus === "hidden"
    ? (review?.isFa ? "نظر شما پنهان است" : "Your review is hidden")
    : (review?.isFa ? "نظر شما در انتظار بررسی است" : "Your review is awaiting approval");
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-md">
      ...
      {isUnpublishedOwnReview ? <p className="mt-3 inline-flex rounded-full bg-amber-50 px-3 py-1.5 text-xs font-black text-amber-800">{reviewStatusLabel}</p> : null}
