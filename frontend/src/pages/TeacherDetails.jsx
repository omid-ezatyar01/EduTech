...
import { fetchPendingCourseRatings, fetchStudentEnrollments, fetchStudentRatings } from "../../services/courseService.js";
...
export default function TeacherDetails({ language = "fa" }) {
  ...
  const refreshRatingAccess = useCallback(async () => {
    if (!getToken()) return;
  ...
  useEffect(() => {
    if (!teacher?._id || !getToken()) return;
    // Load protected review eligibility when the public teacher changes.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    refreshRatingAccess();
  }, [teacher?._id, refreshRatingAccess]);
              ...
              {data.sections.endedCourses.map((course, index) => (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <Link to={buildCoursePath(course)} className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-primary-200 bg-white px-4 text-sm font-black text-primary-700 transition hover:bg-primary-50">
                      {isFa ? "مشاهده جزئیات و نظرها" : "View details and reviews"}
                    </Link>
                  </div>
                </article>
              ))}
