...
const translations = {
  fa: {
    ...
    cta: "انتخاب اولین کورس", soon: "زبان‌های بیشتر به‌زودی", english: "انگلیسی", overview: "مسیر در یک نگاه", overviewText: "روی هر مرحله بزنید تا نقطه شروع مناسب خود را بررسی کنید.", completePath: "۴ مرحله · از پایه تا پیشرفته",
    choices: [
  ...
  en: {
    ...
    cta: "Choose my first course", soon: "More languages soon", english: "English", overview: "Path at a glance", overviewText: "Select a stage to review the starting point that fits you.", completePath: "4 stages · Foundations to advanced",
    choices: [
...
export default function EnglishRoadmapPage({ language = "fa" }) {
    ...
    async function loadCourses() {
      try {
        ...
        if (!cancelled) {
          ...
          setError(
            language === "fa"
              ? "بارگذاری کورس‌های نقشه راه انجام نشد."
              : "Unable to load roadmap courses.",
          );
        }
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-10 pt-8 text-slate-950" dir={language === "fa" ? "rtl" : "ltr"}>
      <div className="mx-auto max-w-[1536px] px-4 sm:px-6 lg:px-8">
        <header className="relative overflow-hidden rounded-3xl border border-slate-100 bg-white px-5 py-8 shadow-sm sm:px-8 sm:py-10 lg:px-12 lg:py-12">
          ...
          <div className="relative mx-auto grid max-w-[1340px] items-center gap-9 lg:grid-cols-[minmax(0,1fr)_430px] lg:gap-12">
            <div>
              <div className="mb-5 flex flex-wrap items-center gap-3">
                <Link to="/roadmaps" className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/80 px-3.5 py-2 text-xs font-black text-slate-600 transition hover:border-primary-200 hover:text-primary-700">
                  <LayoutGrid size={15} />{page.allRoadmaps}
                </Link>
                <span className="inline-flex items-center gap-2 rounded-full border border-teal-200 bg-teal-50 px-3.5 py-2 text-xs font-black text-teal-700"><Route size={16} />{page.eyebrow}</span>
              </div>
              <h1 className="max-w-3xl text-3xl font-black leading-[1.25] tracking-tight text-slate-950 sm:text-5xl lg:text-[3.4rem]">{page.title}</h1>
              <p className="mt-5 max-w-2xl text-sm font-medium leading-7 text-slate-600 sm:text-lg sm:leading-8">{page.intro}</p>
              <div className="mt-7 flex flex-wrap gap-3">
                <button type="button" onClick={() => document.getElementById("level-finder")?.scrollIntoView({ behavior: "smooth", block: "center" })} className="inline-flex h-12 items-center gap-2 rounded-xl bg-primary-600 px-5 text-sm font-black text-white shadow-lg shadow-primary-100 transition hover:bg-primary-700 focus:outline-none focus:ring-4 focus:ring-primary-100">
                  {page.start}<ChevronDown size={17} />
                </button>
                <Link to={buildEnglishCatalogPath()} className="inline-flex h-12 items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 text-sm font-black text-slate-700 transition hover:border-primary-200 hover:text-primary-700">
                  {page.courses}<DirectionArrow size={17} />
                </Link>
              </div>
            </div>

            <div className="rounded-3xl border border-slate-200 bg-white/85 p-4 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur sm:p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-black text-slate-950">{page.overview}</p>
                  <p className="mt-1 text-xs font-medium leading-5 text-slate-500">{page.overviewText}</p>
                </div>
                <span className="shrink-0 rounded-full bg-teal-50 px-3 py-1.5 text-[10px] font-black text-teal-700">{page.completePath}</span>
              </div>
              <div className="relative mt-5 space-y-2 before:absolute before:bottom-5 before:start-[19px] before:top-5 before:w-px before:bg-slate-200">
                {page.stages.map((stage, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative z-10 flex w-full items-center gap-3 rounded-2xl border p-3 text-start transition focus:outline-none focus:ring-4 focus:ring-primary-100 ${active ? "border-primary-200 bg-primary-50 shadow-sm" : "border-transparent bg-white/80 hover:border-slate-200"}`}
                    >
                      <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl text-xs font-black ${active ? "bg-primary-600 text-white" : "border border-slate-200 bg-white text-slate-500"}`}>{stage[0]}</span>
                      <span className="min-w-0 flex-1"><span className={`block truncate text-sm font-black ${active ? "text-primary-800" : "text-slate-800"}`}>{stage[2]}</span><span className="mt-0.5 block text-[11px] font-bold text-slate-500">{stage[1]}</span></span>
                      <span className={`rounded-lg px-2.5 py-1.5 text-[10px] font-black ${active ? "bg-white text-primary-700" : "bg-slate-100 text-slate-600"}`}>{stage[3]}</span>
                    </button>
                  );
                })}
              </div>
        ...
        {error ? (
          <div className="mt-5 flex flex-col items-center justify-between gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-center sm:flex-row sm:text-start">
            ...
            <button
              ...
              className="rounded-xl bg-white px-4 py-2 text-sm font-black text-rose-700 shadow-sm"
            >
      ...
      <section id="level-finder" className="scroll-mt-24 px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        <div className="mx-auto max-w-[1340px] rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8 lg:p-10">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {page.choices.map((choice, index) => (
              <button key={choice} type="button" onClick={() => chooseStage(index)} className={`flex min-h-24 items-start gap-3 rounded-2xl border p-4 text-start text-sm font-bold leading-6 transition focus:outline-none focus:ring-4 focus:ring-primary-100 sm:p-5 ${selectedStage === index ? "border-primary-400 bg-primary-50 text-primary-900 shadow-sm" : "border-slate-200 bg-slate-50/60 text-slate-700 hover:border-primary-200 hover:bg-white"}`}>
            ...
            {page.stages.map(([number, label, title, level, duration, description, skills], index) => {
              ...
              return (
                <article id={`roadmap-stage-${index}`} key={number} className="relative grid grid-cols-[56px_minmax(0,1fr)] gap-4 scroll-mt-32 sm:grid-cols-[80px_minmax(0,1fr)] sm:gap-6">
                  <div className={`relative z-10 grid h-14 w-14 place-items-center rounded-2xl border-4 border-white shadow-sm sm:h-20 sm:w-20 ${selected ? "bg-primary-600 text-white" : "bg-slate-100 text-slate-600"}`}><Icon size={26} /></div>
                  <div className={`overflow-hidden rounded-3xl border bg-white transition ${selected ? "border-primary-300 shadow-[0_18px_45px_rgba(37,99,235,0.10)]" : "border-slate-200 shadow-sm"}`}>
                    <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[minmax(0,1fr)_220px]">
                      <div>
                        ...
                        <button
                          ...
                          className="mt-5 inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 px-4 text-xs font-black text-slate-700 transition hover:border-primary-200 hover:text-primary-700"
                        >
                    ...
                    <div className="border-t border-slate-100 bg-slate-50 p-4 sm:px-7 sm:py-5">
                      {loading ? <p className="text-sm font-bold text-slate-500">{page.loading}</p> : error ? <p className="text-sm font-bold text-rose-600">{error}</p> : stageCourses.length ? (
                        <><div className="grid gap-3 lg:grid-cols-2">{stageCourses.map((course) => (
                          <Link key={course._id || course.id} to={buildCoursePath(course)} className="group flex min-w-0 items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3 transition hover:border-primary-300 hover:shadow-sm">
                            <div className="h-14 w-16 shrink-0 overflow-hidden rounded-xl bg-primary-50">{course.thumbnail ? <img src={course.thumbnail} alt="" className="h-full w-full object-cover" /> : <span className="grid h-full w-full place-items-center text-primary-600"><BookOpen size={21} /></span>}</div>
      ...
      <section className="border-t border-slate-200 py-12 sm:py-14">
        <div className="mx-auto grid max-w-[1340px] gap-8 px-4 sm:px-6 lg:grid-cols-[340px_minmax(0,1fr)] lg:px-8">
          ...
          <div className="grid gap-px overflow-hidden rounded-3xl border border-slate-200 bg-slate-200 shadow-sm md:grid-cols-3">
            {page.selectionSteps.map(([title, text], index) => (
              <div key={title} className="bg-white p-5 sm:p-6">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-primary-50 text-sm font-black text-primary-700">{index + 1}</span>
      ...
      <section className="border-y border-slate-200 bg-white py-12 sm:py-14">
        <div className="mx-auto grid max-w-[1340px] gap-8 px-4 sm:px-6 lg:grid-cols-[320px_minmax(0,1fr)] lg:items-center lg:px-8">
          <div><div className="grid h-12 w-12 place-items-center rounded-2xl bg-teal-100 text-teal-700"><Sparkles size={22} /></div><h2 className="mt-4 text-2xl font-black sm:text-3xl">{page.habitTitle}</h2><p className="mt-3 text-sm font-medium leading-7 text-slate-600">{page.habitText}</p></div>
          <div className="grid grid-cols-2 gap-px overflow-hidden rounded-3xl border border-slate-200 bg-slate-200 shadow-sm sm:grid-cols-4">
      ...
      <div className="mx-auto mt-8 max-w-[1536px] px-4 sm:px-6 lg:px-8">
        <section className="relative overflow-hidden rounded-3xl bg-primary-700 py-10 text-white sm:py-12"><div className="absolute -end-16 -top-28 h-64 w-64 rounded-full bg-teal-400/20 blur-3xl" /><div className="relative mx-auto flex max-w-[1340px] flex-col gap-6 px-5 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:px-12"><div><h2 className="text-2xl font-black sm:text-3xl">{page.ctaTitle}</h2><p className="mt-2 text-sm font-medium leading-7 text-blue-100 sm:text-base">{page.ctaText}</p></div><Link to={buildEnglishCatalogPath(selectedStage)} className="inline-flex h-12 shrink-0 items-center justify-center gap-2 self-start rounded-xl bg-white px-5 text-sm font-black text-primary-700 transition hover:bg-teal-50 lg:self-auto">{page.cta}<DirectionArrow size={17} /></Link></div></section>
      </div>
    </div>
  );
}
