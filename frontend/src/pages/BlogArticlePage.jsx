...
export default function BlogArticlePage({ language = "fa" }) {
  ...
  return <article className="min-h-screen bg-slate-50 pb-16" dir={locale === "fa" ? "rtl" : "ltr"}>
    ...
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-14"><div className="flex flex-wrap items-center gap-2 text-sm font-bold text-slate-400"><Link to="/blog" className="text-blue-700">{page.blog}</Link><Arrow size={15}/><span>{categoryNames[locale][article.category] || article.category}</span></div><h1 className="mt-6 text-3xl font-black leading-tight text-slate-950 sm:text-5xl">{title}</h1><p className="mt-5 whitespace-pre-wrap break-words text-justify text-base font-medium leading-8 text-slate-600 sm:text-lg">{localized(article.excerpt, locale)}</p><div className="mt-7 flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-4 text-sm font-bold text-slate-500"><span>{article.author?.name || "EduTech"}</span><span className="inline-flex items-center gap-1"><Clock3 size={16}/>{article.estimatedReadMinutes} {page.minutes}</span><span className="inline-flex items-center gap-1"><Eye size={16}/>{Number(article.viewCount || 0).toLocaleString(locale === "fa" ? "fa-AF" : "en-US")}</span></div><button onClick={share} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-black text-slate-700"><Share2 size={17}/>{page.share}</button></div></div></header>
    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6"><div className="whitespace-pre-wrap break-words text-justify text-base font-medium leading-9 text-slate-700 sm:text-lg">{content}</div></div>
